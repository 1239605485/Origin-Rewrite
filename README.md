...
2. 每个 SetDefaults→AI 生命周期只允许一次 `or_spawn_try_commit()`；已知的拒绝结果不会在后续 AI tick 重抽。`active`、`NPC.value` 和 `Main.netMode` 属于可选兼容字段，不会阻止核心生命/伤害/防御属性提交。验证版不在 SetDefaults 阶段使用活动精英上限，避免模板对象占满名额。
