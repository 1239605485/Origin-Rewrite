...
当前 `0.4.1-visual-method-signature`（versionCode `2026090442`）在上一版基础上继续只读解析 `NPC.color`，并安全枚举 `NPC` 暴露的字段、属性和方法名称及方法签名，记录可能与颜色、绘制、透明度、发光、特效或拖尾相关的成员；扫描不会调用未知方法，也不会写入对象。重构体名称、属性和已确认的 AI 兼容分类保持不变；仅对“灾变体/终焉体”在真实提交成功后调用已通过 ABI 探测的 `Main.NewText` 做一次系统播报，异化体不播报。颜色、NPCLoot、额外掉落和特殊 AI 仍关闭。此版本继续写入独立的 `originrewrite_runtime.log`：同时写入模组私有目录和 TEFKernel 导出目录，并通过 `OriginRewrite` logcat 标签输出启动阶段。
