...
当前 `0.3.5-lifecycle-test`（versionCode `2026090436`）在上一版基础上增加长时间刷怪诊断：每约 30 秒输出一次 `[LIFECYCLE_HEALTH]`，汇总绑定表占用、pending/elite 数量、SetDefaults/AI/提交次数、回收/满表次数和状态清理次数。重构体名称、属性和已确认的 AI 兼容分类保持不变；仅对“灾变体/终焉体”在真实提交成功后调用已通过 ABI 探测的 `Main.NewText` 做一次系统播报，异化体不播报。颜色、NPCLoot、额外掉落和特殊 AI 仍关闭。此版本继续写入独立的 `originrewrite_runtime.log`：同时写入模组私有目录和 TEFKernel 导出目录，并通过 `OriginRewrite` logcat 标签输出启动阶段。
