...
当前 `0.3.0-name-display-source`（versionCode `2026090431`）按设计文档 v0.5 调整生成顺序：`SetDefaults` 只记录 pending 和原版基准，只有已精确校验并安装的 `NPC.AI()` Postfix 在观察到 `active=true` 后才允许一次 `SpawnCommitted`。首次提交后只尝试写入一次 `GivenName` 名称前缀；敌怪没有自定义 GivenName 时，优先从严格校验的 `FullName`/`TypeName` getter 取得原版显示名，再写入“前缀·原名”，并记录回读阶段。颜色、`Main.NewText`、NPCLoot、额外掉落和特殊 AI 仍关闭。此版本继续写入独立的 `originrewrite_runtime.log`：同时写入模组私有目录和 TEFKernel 导出目录，并通过 `OriginRewrite` logcat 标签输出启动阶段。
...
- `or_adapter.c`：只安装精确校验的 `NPC.SetDefaults(int,pointer)` 观察 Hook 和 `NPC.AI()` Postfix；SetDefaults 只保存 pending，AI 首次确认 active 后才提交；提交后仅尝试一次 `GivenName` 重构体前缀，颜色、公告、NPCLoot、额外掉落和特殊 AI 仍关闭。
