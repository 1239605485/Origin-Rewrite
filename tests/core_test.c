...
static void test_bnm_compatibility_gate(void) {
    CHECK(or_compat_bnm_unity_supported("2021.3.56f2"));
    CHECK(or_compat_bnm_unity_supported("2022.2.21f1"));
    CHECK(or_compat_bnm_unity_supported("2022.3.62f1"));
    CHECK(!or_compat_bnm_unity_supported("6000.0.10f1"));
    CHECK(!or_compat_bnm_unity_supported(NULL));
    CHECK(!or_compat_bnm_unity_supported(""));
}

static void test_general_json_overlay(void) {
    static const char json[] =
        "{\"enableElites\":false,\"maxActiveRewrites\":12,"
        "\"classicChance\":0.25,\"expertChance\":0.35,"
        "\"allowPreHardmodeApocalypse\":true}";
    OR_Config config;
    OR_ConfigIoReport report;
    or_config_default(&config);
    CHECK(or_config_io_apply_json(json, &config, &report));
    CHECK(report.parsed);
    CHECK(report.overrides_applied == 5u);
    CHECK(!config.enable_elites);
    CHECK(config.max_active_elites == 12u);
    CHECK(config.modes[OR_MODE_CLASSIC].elite_chance == 0.25f);
    CHECK(config.modes[OR_MODE_EXPERT].elite_chance == 0.35f);
    CHECK(config.allow_pre_hardmode_apocalypse);
    CHECK(or_config_validate(&config));
}
...
static void test_biome_modifiers(void) {
    OR_Config config;
    OR_RuleSnapshot forest;
    OR_RuleSnapshot jungle;
    OR_RuleSnapshot snow;
    OR_RuleSnapshot crimson;
    or_config_default(&config);
    CHECK(or_config_validate(&config));
    CHECK(or_rules_build_snapshot(&config, OR_PROGRESS_HARDMODE_PRE_MECH,
                                  test_terrain(), OR_WEATHER_CLEAR, false,
                                  101u, &forest));
    CHECK(or_rules_build_snapshot(&config, OR_PROGRESS_HARDMODE_PRE_MECH,
                                  (OR_TerrainSnapshot){OR_DEPTH_SURFACE, OR_BIOME_JUNGLE, OR_SPECIAL_NONE},
                                  OR_WEATHER_CLEAR, false, 101u, &jungle));
    CHECK(or_rules_build_snapshot(&config, OR_PROGRESS_HARDMODE_PRE_MECH,
                                  (OR_TerrainSnapshot){OR_DEPTH_SURFACE, OR_BIOME_SNOW, OR_SPECIAL_NONE},
                                  OR_WEATHER_CLEAR, false, 101u, &snow));
    CHECK(or_rules_build_snapshot(&config, OR_PROGRESS_HARDMODE_PRE_MECH,
                                  (OR_TerrainSnapshot){OR_DEPTH_SURFACE, OR_BIOME_CRIMSON, OR_SPECIAL_NONE},
                                  OR_WEATHER_CLEAR, false, 101u, &crimson));
    CHECK(jungle.life_multiplier > forest.life_multiplier);
    CHECK(jungle.damage_multiplier > forest.damage_multiplier);
    CHECK(snow.defense_multiplier > forest.defense_multiplier);
    CHECK(crimson.damage_multiplier > forest.damage_multiplier);
    CHECK(forest.terrain.biome == OR_BIOME_FOREST);
    CHECK(jungle.life_multiplier <= config.caps.rule_life_damage_defense_max);
    CHECK(crimson.damage_multiplier <= config.caps.rule_life_damage_defense_max);
}
...
int main(void) {
    ...
    test_bnm_compatibility_gate();
    test_general_json_overlay();
    test_rule_caps();
