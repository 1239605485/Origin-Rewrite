...
static kernel_mod_info_t g_mod_info = {
    ...
    .version_code = 2026090405,
    .api_version = 1,
    .version = "1.0.4-hook-observable"
};
...
static void fw_init_mod(kernel_mod_handle_t *handle) {
    ...
    __android_log_print(ANDROID_LOG_INFO, "OriginRewrite",
                        "[MODULE_BEACON] version=1.0.4-hook-observable "
                        "versionCode=2026090405");
#endif
    ready = fw_core_init();
    if (mod_logger_write) {
        mod_logger_write(MOD_LOG_LEVEL_INFO, "OriginRewrite",
                         "Origin Rewrite(起源重构) v1.0.4-hook-observable "
