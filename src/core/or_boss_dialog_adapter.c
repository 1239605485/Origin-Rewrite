...
void or_boss_dialog_adapter_requeue(OR_BossDialogAdapter *state, OR_BossDialogEvent event) { if (!state) return; if (event == OR_BOSS_DIALOG_SPAWN) state->seen = false; else if (event == OR_BOSS_DIALOG_HALF) state->half_announced = false; }
