...
static void or_apply_context_modifiers(OR_RuleSnapshot *snapshot) {
    ...
    switch (snapshot->terrain.biome) {
        case OR_BIOME_DESERT:
            snapshot->damage_multiplier *= 1.08f;
            snapshot->tier_weight_multiplier[OR_TIER_CALAMITY] *= 1.05f;
            break;
        case OR_BIOME_SNOW:
            snapshot->defense_multiplier *= 1.08f;
            snapshot->damage_multiplier *= 0.96f;
            break;
        case OR_BIOME_JUNGLE:
            snapshot->life_multiplier *= 1.06f;
            snapshot->damage_multiplier *= 1.06f;
            snapshot->tier_weight_multiplier[OR_TIER_CALAMITY] *= 1.08f;
            break;
        case OR_BIOME_HALLOW:
            snapshot->life_multiplier *= 1.06f;
            snapshot->defense_multiplier *= 1.05f;
            snapshot->tier_weight_multiplier[OR_TIER_CALAMITY] *= 1.10f;
            break;
        case OR_BIOME_CORRUPTION:
            snapshot->life_multiplier *= 1.08f;
            snapshot->damage_multiplier *= 1.08f;
            snapshot->tier_weight_multiplier[OR_TIER_CALAMITY] *= 1.10f;
            break;
        case OR_BIOME_CRIMSON:
            snapshot->life_multiplier *= 1.08f;
            snapshot->damage_multiplier *= 1.10f;
            snapshot->defense_multiplier *= 1.04f;
            snapshot->tier_weight_multiplier[OR_TIER_CALAMITY] *= 1.10f;
            break;
