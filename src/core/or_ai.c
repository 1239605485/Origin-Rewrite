...
const char *or_ai_phase_name(OR_AiPhase phase) {
    switch (phase) {
        case OR_AI_PHASE_TELEGRAPH: return "telegraph";
        case OR_AI_PHASE_ACTIVE: return "active";
        case OR_AI_PHASE_RECOVERY: return "recovery";
        case OR_AI_PHASE_COOLDOWN: return "cooldown";
...
OR_AiArchetype or_ai_classify_native_style(int32_t ai_style, bool *known) {
    ...
    switch (ai_style) {
        ...
        case 8:  /* Caster AI; keep spellcasters out of the melee bucket. */
            return OR_AI_ARCHETYPE_SPECIAL;
        case 9:  /* Spell/projectile AI; remain non-melee until ABI is verified. */
            return OR_AI_ARCHETYPE_SPECIAL;
        case 3:  /* Fighter AI. */
            return OR_AI_ARCHETYPE_MELEE;
        default:
...
OR_AiArchetype or_ai_classify_native_type(uint32_t npc_type,
                                          int32_t ai_style,
                                          bool *known) {
    /* Terraria's Goblin Archer (NPC type 111) uses the shared fighter
     * aiStyle=3, but its native behavior is projectile-based. This verified
     * type exception must run before the generic style fallback. */
    if (npc_type == 111u) {
        if (known) *known = true;
        return OR_AI_ARCHETYPE_RANGED;
    }
    return or_ai_classify_native_style(ai_style, known);
}
