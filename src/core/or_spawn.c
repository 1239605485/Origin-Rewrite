...
bool or_spawn_try_commit(const OR_Config *config,
    ...
    if (config->apocalypse_only_test && config->tiers[OR_TIER_APOCALYPSE].enabled &&
        (context->progress != OR_PROGRESS_PRE_HARDMODE || config->allow_pre_hardmode_apocalypse)) {
        tier_weights[OR_TIER_ALTERED] = 0.0f;
        tier_weights[OR_TIER_CALAMITY] = 0.0f;
        tier_weights[OR_TIER_APOCALYPSE] = 1.0f;
    }
