...
bool or_config_validate(OR_Config *config) {
    ...
    for (i = 0; i < OR_PROGRESS_COUNT; ++i) {
        ...
        for (mode = 0; mode < OR_MODE_COUNT; ++mode) {
            ...
            if (config->apocalypse_only_test && config->tiers[OR_TIER_APOCALYPSE].enabled &&
                (i != OR_PROGRESS_PRE_HARDMODE || config->allow_pre_hardmode_apocalypse)) {
                progress->tier_weight[mode][OR_TIER_ALTERED] = 0.0f;
                progress->tier_weight[mode][OR_TIER_CALAMITY] = 0.0f;
                progress->tier_weight[mode][OR_TIER_APOCALYPSE] = 1.0f;
            }
        }
