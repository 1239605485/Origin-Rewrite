...
/* Terraria Android target: Unity 2021.3.56f2. This is a build-time ABI
 * selection, not a runtime preference. */
...
// OriginRewrite only consumes existing IL2CPP metadata.  Disabling class
// creation avoids BNM's secondary native hook layer; TEFKernel/PatchLib owns
// all runtime hook installation and uninstallation.
...
// OriginRewrite metadata-only backend.  BNM's native hook API is intentionally
// inert: this build never calls TryLoadBy* or BasicHook.  BNM is initialized
// from TEFKernel's already-established IL2CPP thread via
// TrySetupByUsersFinder(), while gameplay hooks remain PatchLib-owned.
